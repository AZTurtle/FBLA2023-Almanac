...
Sprite3D::Sprite3D(const char* file, glm::mat4* transform, Camera* cam, float* color){
    img = assets.getTexture(file);
    shader = new Shader(shaders::diffusesprite3D, uniforms{transform, color, img, &cam->matrix}, staticUniforms{});
    float w = img->w / 2.0f, h = img->h / 2.0f;
    if(w > h){
        h /= w;
        w = 1.0f;
